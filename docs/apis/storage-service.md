...
> 표기 — **굵은 파트 = 필수**. 모든 엔드포인트는 Spring API `version=1.0.0` 단일 버전 — `/api/v1` 와 `/api/v2` 는 URL 경로 구분일 뿐 버전 협상값이 아니다.
...
| Header | Value | Notes |
|---|---|---|
| `api-version` | `1.0.0` | Spring API versioning 값 (v1·v2 공통) |
| `Content-Type` | `multipart/form-data` | 모든 POST 는 `@ModelAttribute` form 바인딩 (JSON 아님) |

**Security** — `@PreAuthorize` 없음. 인증은 게이트웨이 / Keycloak 에서 강제. 컨트롤러에 `@RequestHeader` 없음 — 커스텀 필수 헤더 없음.

**두 API surface**

- `/api/v1/*` — **deprecated** (`@Deprecated(since=2026-03-26, forRemoval=false)`, 여전히 존재). 각 응답에 deprecation 헤더 부착 — `Deprecation: true`, `Sunset: Wed, 31 Dec 2026 23:59:59 GMT`, `Link: </api/v2/upload>; rel="successor-version"`. 헤더는 v1 컨트롤러의 private helper `applyV1DeprecationHeaders(...)` 가 메서드별로 부착 (인터셉터/필터 아님).
- `/api/v2/*` — current. deprecation 헤더 없음. pre-sign / confirm-upload 추가.

**Common response envelope** (`CncResponse`) — 성공 응답의 바깥 틀. `ResponseHelper.success(HttpStatus.OK, data, null)` 는 null 필드를 생략하므로 성공 시 `token`/`code`/`message`/`error` 는 빠진다.
...
**Error envelope** (`ErrorResponse`) — `CncErrorResponseBuilder.toResponseEntity(error)`. HTTP status 는 `ErrorCode` 에서. throw 경로(`GlobalExceptionHandler`)와 byte-identical.
...
> 에러 코드는 common-core `ErrorCode`(**`CMN-*`** prefix) — storage 전용 `STR-*` 네임스페이스는 존재하지 않는다. 전체는 문서 하단 [## 에러 코드](#에러-코드) 참조.

---

## API 버전 (endpoint별)

> `version` 열은 Spring API versioning 값. storage-service 는 v1·v2 모든 엔드포인트가 `1.0.0`. v1/v2 구분은 URL 경로.

| Method | Path | 제공 버전 | 최신 | 비고 |
|---|---|---|---|---|
| POST | /api/v1/upload | 1.0.0 | 1.0.0 | deprecated |
| GET | /api/v1/download | 1.0.0 | 1.0.0 | deprecated |
| DELETE | /api/v1/delete | 1.0.0 | 1.0.0 | deprecated |
...
## 1. `POST` /api/v1/upload (deprecated)
...
## 2. `GET` /api/v1/download (deprecated)
...
## 3. `DELETE` /api/v1/delete (deprecated)
...
`multipart/form-data` — `@Valid @ModelAttribute FileUploadRequest` (§1 의 `AwsUploadRequest` 와 필드 동일).
...
## 6. `GET` /api/v2/pre-sign/download
