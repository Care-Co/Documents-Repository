...
| `device_number` | `VARCHAR(20)` | no | 사용자/조직 친화 번호 (예. `FS2-001`). owner 별 type 별 누적 (V15, 0.0.73). UNCLAIMED 는 NULL |
...
| `uk_devices_owner_device_number` | `owner_type`, `owner_id`, `device_number` | **partial unique** `WHERE device_number IS NOT NULL` (V15). 같은 owner 안에서 device_number 중복 방지 |

### 8.2.1 `device_number_sequence` (V15, 0.0.73)

owner 별 type 별 누적 순번 — `DeviceNumberGenerator` 가 PESSIMISTIC_WRITE 락으로 조회·증가시킨다.

| 컬럼 | 타입 | 필수 | 비고 |
|---|---|---|---|
| `owner_type` | `VARCHAR(20)` | yes | composite PK |
| `owner_id` | `VARCHAR(36)` | yes | composite PK |
| `type_key` | `VARCHAR(10)` | yes | composite PK. `FS2` (SCALE2) / `FZ` (SCALE_PUZZLE) |
| `last_seq` | `BIGINT` | yes | 0 시작. `next()` 시 +1 |

PK = `(owner_type, owner_id, type_key)`. 같은 owner 의 두 번째 SCALE2 등록 시 `FS2-002` 자동 부여.
