...
| Base path | 모든 REST 엔드포인트 `/api/v1/...` 하위 (내부 gRPC 는 문서 하단 [내부 gRPC](#내부-grpc) 참조) |

---

## 공통 규칙

- **버전** — 모든 REST 엔드포인트 versioned. 버전 협상은 요청 헤더 `api-version: x.y.z` (Spring API versioning) — device-service 는 전 엔드포인트가 `1.0.0` 단일 버전 (미등록 버전 → `400 CMN-400-001`).
- **인증** — `@PreAuthorize` 없음. 인증은 게이트웨이에서 강제. device 엔드포인트는 `X-Caller-Type`/`X-Caller-Id` 로 owner 를 식별하고, admin 엔드포인트는 `X-Caller-Role: ADMIN` 을 서비스 레이어에서 검사 (`AllowedDeviceError.Forbidden` → 403).
- **바디** — 요청 바디가 있으면 `Content-Type: application/json` (bulk 는 `multipart/form-data`).
- **응답 틀** — 모든 응답은 `CncResponse` envelope (아래 접기 참조) — 개별 섹션 샘플에서 반복하지 않음.

<details>
<summary><b>공통 헤더</b> — 전 엔드포인트 공통</summary>
...
**성공** (`CncResponse`) — 모든 응답 샘플의 바깥 틀. 아래 필드는 모든 엔드포인트 공통이라 개별 섹션에서 반복하지 않는다.
...
|---|---|---|---|---|
| POST | [`/devices`](#1-post-apiv1devices) | 1.0.0 | `X-Caller-*` | caller 본인의 device 등록 |
| GET | [`/devices/{id}`](#2-get-apiv1devicesid) | 1.0.0 | — | device 단건 조회 |
| GET | [`/devices`](#3-get-apiv1devices) | 1.0.0 | `X-Caller-*` | caller 가 소유한 device 목록 (REVOKED 포함 — 감사용 보존) |
| PATCH | [`/devices/{id}/status`](#4-patch-apiv1devicesidstatus) | 1.0.0 | `X-Caller-*` | caller 본인 device 의 상태 변경 |
| POST | [`/devices/validate`](#5-post-apiv1devicesvalidate) | 1.0.0 | `X-Caller-*` | `(macAddress, hardwareSerial)` 가 caller 소유 + `ACTIVE` 인지 검증 |
| DELETE | [`/devices/{id}`](#6-delete-apiv1devicesid) | 1.0.0 | `X-Caller-*` | caller 본인 device unregister |
...
## 4. `PATCH` /api/v1/devices/{id}/status
...
## 5. `POST` /api/v1/devices/validate
...
## admin allowed-devices

| Method | Path (`/api/v1` 이하) | 버전 | 인증 | 설명 |
|---|---|---|---|---|
| POST | [`/admin/allowed-devices`](#7-post-apiv1adminallowed-devices) | 1.0.0 | ADMIN | 출고 풀에 단건 추가 |
| POST | [`/admin/allowed-devices/bulk`](#8-post-apiv1adminallowed-devicesbulk) | 1.0.0 | ADMIN | xlsx 일괄 업로드 |
| GET | [`/admin/allowed-devices/{id}`](#9-get-apiv1adminallowed-devicesid) | 1.0.0 | ADMIN | 풀 단건 조회 |
| GET | [`/admin/allowed-devices`](#10-get-apiv1adminallowed-devices) | 1.0.0 | ADMIN | 풀 목록 (페이지) |
| PATCH | [`/admin/allowed-devices/{id}/serial`](#11-patch-apiv1adminallowed-devicesidserial) | 1.0.0 | ADMIN | provisional row 의 진짜 시리얼 확정 |
| POST | [`/admin/allowed-devices/{id}/unclaim`](#12-post-apiv1adminallowed-devicesidunclaim) | 1.0.0 | ADMIN | 풀 row 강제 unclaim + 바인딩된 device 가 있으면 REVOKED |
...
## 8. `POST` /api/v1/admin/allowed-devices/bulk
...
## 9. `GET` /api/v1/admin/allowed-devices/{id}
...
## 10. `GET` /api/v1/admin/allowed-devices
...
## 12. `POST` /api/v1/admin/allowed-devices/{id}/unclaim
...
| Method | Path (`/api/v1` 이하) | 버전 | 인증 | 설명 |
|---|---|---|---|---|
| POST | [`/devices/firmware`](#13-post-apiv1devicesfirmware) | 1.0.0 | — | 펌웨어 메타 등록 |
| GET | [`/devices/firmware`](#14-get-apiv1devicesfirmware) | 1.0.0 | — | deviceType 별 펌웨어 목록 |
| GET | [`/devices/firmware/{id}`](#15-get-apiv1devicesfirmwareid) | 1.0.0 | — | 펌웨어 단건 조회 |
| PATCH | [`/devices/firmware/{id}/promote`](#16-patch-apiv1devicesfirmwareidpromote) | 1.0.0 | — | 선택한 펌웨어를 해당 `deviceType` 의 latest 로 promote |
| GET | [`/devices/firmware/upgrade`](#17-get-apiv1devicesfirmwareupgrade) | 1.0.0 | — | device 측 polling endpoint — 최신과 다르면 최신 펌웨어 반환 |
| DELETE | [`/devices/firmware/{id}`](#18-delete-apiv1devicesfirmwareid) | 1.0.0 | — | 펌웨어 row 삭제 (soft — `deletedAt` 마킹) |

---

## 13. `POST` /api/v1/devices/firmware
...
## 14. `GET` /api/v1/devices/firmware
...
배열 각 항목이 §13 의 `FirmwareResponse` (9 필드).

---

## 15. `GET` /api/v1/devices/firmware/{id}
...
## 16. `PATCH` /api/v1/devices/firmware/{id}/promote
...
## 17. `GET` /api/v1/devices/firmware/upgrade
